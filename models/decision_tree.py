...
# Cap training data — encoding + 6 ensemble models on 50K rows is the bottleneck
SAMPLE_CAP = 10_000
...
def _run_tree_models(df):
    ...
    # 2. Bagging — reduced estimators, parallel
    bagging = BaggingClassifier(n_estimators=20, random_state=config.RANDOM_STATE, n_jobs=-1)
    bagging.fit(x_train, y_train)
    rf = RandomForestClassifier(n_estimators=50, random_state=config.RANDOM_STATE, n_jobs=-1)
    rf.fit(x_train, y_train)

    # 3. Boosting — reduced estimators

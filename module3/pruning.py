    ...
    for criterion in ['gini', 'entropy']:
        for max_depth in [None, 3, 5, 10]:
            ...
            results.append({
                ...
                'gap': round(float(accuracy_score(ytr, dt.predict(Xtr)) - accuracy_score(yva, dt.predict(Xva))) * 100, 2),
            })

    # min_samples_leaf pruning experiment
    leaf_results = []
    for msl in [1, 5, 10, 20, 50]:
        dt = DecisionTreeClassifier(
            min_samples_leaf=msl, random_state=config.RANDOM_STATE
        )
        dt.fit(Xtr, ytr)
        leaf_results.append({
